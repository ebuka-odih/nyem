...
class BuyRequestController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Verify item is a marketplace item
        if ($item->type !== 'marketplace') {
            return response()->json([
                'success' => false,
                'message' => 'Buy requests are only available for marketplace items',
            ], 422);
        }
        ...
        if ($item->user_id === $user->id) {
            return response()->json([
                'success' => false,
                'message' => 'You cannot send a buy request for your own item',
            ], 422);
        }
        ...
        // Create or update buy request (ensures one request per buyer-seller-item)
        ...
        return response()->json([
            ...
            'data' => [
                'buy_request' => $buyRequest->load(['buyer', 'item']),
            ],
        ], 201);
    }
    ...
    public function pending(Request $request)
    {
        ...
        $filteredRequests = $buyRequests->filter(function ($buyRequest) use ($blockedUserIds) {
            ...
            return [
                ...
                'item' => [
                    'id' => $buyRequest->item->id,
                    'title' => $buyRequest->item->title,
                    'photo' => !empty($buyRequest->item->photos) ? $buyRequest->item->photos[0] : null,
                ],
    ...
    public function respond(Request $request, string $id)
    {
        ...
        $user = $request->user();
        $buyRequest = BuyRequest::with(['buyer', 'item'])->findOrFail($id);
        ...
        return response()->json([
            ...
            'data' => [
                'buy_request' => $buyRequest->fresh()->load(['buyer', 'item']),
