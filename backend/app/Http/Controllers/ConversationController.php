...
class ConversationController extends Controller
{
    ...
    public function start(Request $request)
    {
        ...
        // If item_id is provided, check if it's a marketplace item
        // For marketplace items, create a message request instead of immediate conversation
        if (!empty($data['item_id'])) {
            $item = Item::findOrFail($data['item_id']);
            $itemType = $item->type ?? 'barter';
            
            // Verify the recipient is the item owner
            if ($item->user_id !== $recipientId) {
                return response()->json([
                    'success' => false,
                    'message' => 'Item does not belong to the specified recipient',
                ], 422);
            }

            // For marketplace items, create a message request
            if ($itemType === 'marketplace') {
                ...
                if (empty($data['message_text']) || trim($data['message_text']) === '') {
                    return response()->json([
                        'success' => false,
                        'message' => 'Message text is required for marketplace items',
                    ], 422);
                }
        ...
        // For barter items or when no item_id is provided, create conversation directly (backward compatibility)
